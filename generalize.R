...
generalize_dr <- function(a, y, x_trial, x_target, sl.lib.source, sl.lib.target, n_folds, mu1_hat = NULL, mu0_hat = NULL, pi_hat = NULL, rho_hat = NULL, tau1_hat = NULL, tau0_hat = NULL, epsilon = NULL, survey = FALSE){
  # a: treatment vector
  # y: outcome vector
  ...
  # mu1_hat: the estiamator for mu1, usually unknown and needs to be estimated
  ...
  for (aa in 1:2){
    ifs[1:n_trial,aa] <- as.numeric(a == aa-1)*(y-muhat[,aa])/(pi_trial*(pi_treat^(aa-1)*(1-pi_treat)^(2-aa))) + 1/pi_trial*(muhat[,aa] - tau_trial[,aa]) + tau_trial[,aa]
...
# Plug-in
generalize_plugin <- function(a, y, x_trial, x_target, sl.lib){
  set.seed(521)
  require(SuperLearner)
  n_trial <- nrow(x_trial)
  n_target <- nrow(x_target)
  ...
  muhat <- matrix(0, nrow=n_trial, ncol=2)
  tau_hat <- matrix(0, nrow=n_trial+n_target, ncol=2)
  for (aa in 1:2){
    mufit <- mcSuperLearner(y[a==aa-1],
                          x_trial[a==aa-1,],
                          newX=x_trial, SL.library=sl.lib)
    muhat[,aa] <- mufit$SL.predict
    taufit <- mcSuperLearner(muhat[,aa], x_trial[,cova_target], newX=rbind(x_trial[,cova_target], x_target), SL.library=sl.lib)
    tau_hat[,aa] <- taufit$SL.predict
  }
  potential_mean <- colMeans(tau_hat)
